/**
 * Changes API — 変更レポート一覧・配信
 *
 * 各プロジェクトの docs/changes/*.md をスキャンし、
...
const log = createLogger('web:changes')
...
/** MD先頭からメタデータをパース */
function parseMeta(content: string, fallbackProject: string): Omit<ChangeReportEntry, 'filename'> {
...
changesRoutes.get('/', (c) => {
  ...
  const reports: ChangeReportEntry[] = []

  for (const p of projects) {
    if (filterProject && p.slug !== filterProject) continue
    const changesDir = join(p.localPath, 'docs', 'changes')
    ...
    try {
      ...
      log.warn(`Failed to read ${changesDir}: ${e}`)
    }
  }
...
changesRoutes.get('/:project/:filename', (c) => {
  ...
  const filePath = join(project.localPath, 'docs', 'changes', filename)
...
changesRoutes.get('/:project/screenshots/:name', (c) => {
  const projectSlug = c.req.param('project')
  ...
  const filePath = join(project.localPath, 'docs', 'changes', 'screenshots', name)
