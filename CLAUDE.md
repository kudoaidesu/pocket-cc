...
本プロジェクトでは **窓口（メインエージェント）は作業を行わない**。

- タスクを受けたら即座にワーカー（Task tool）に割り振る
- 窓口は「受付→割り振り→完了報告」のみ
- 割り振り後は即座に待機状態に戻る（自分のTodoは持たない）
- ワーカー完了時に逐次報告する
...
| Issue | 担当 | ステータス |
|-------|------|-----------|
| #xxx タイトル | worker-N | 作業中/完了 |
```

### 例外（窓口が直接対応してよい場合）
- 1ファイル数行の軽微な修正
- 質問への回答・情報提供
- コミット・PR・マージなどの Git 操作

割り振り精度を上げるため、`task-dispatcher` スキルを参照すること。
